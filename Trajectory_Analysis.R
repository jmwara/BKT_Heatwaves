...
full_trajectory.plot <- ggplot(data=trajectory_pc_plot.df, aes(x=PC1, y=PC2, color=Site))+ #create a ggplot with the PC points for axis 1 and 2 of the trajectory analysis, color the points by site
  ...
  labs(x="PC1 (14.85%)\n <- Ribosomal Components | No Enriched GO Terms ->", y="PC2 (10.86%)\n <- Oxygen Carrier Activity | Immune Response ->", title="A - Trajectory Analysis") #add axis labels 
...
Traj1_temp.plot <- ggplot()+ #create new ggplot object
  geom_path(data=daily_temps.df, aes(x=Date, y=Temp.avg, color=Site), linetype=2, show.legend = F)+ #plot path of daily average temperatures
  geom_point(data=trajectory_traj_plot.df, aes(x=Date, y=(Traj1/12) + 15), size=2, show.legend = F)+ #plot points showing trajectory point value on PC1 during each sampling date (rescaled to fit on same plot as temperature)
  geom_arrow_chain(data=trajectory_traj_plot.df, aes(x=Date, y=(Traj1/12) + 15, color=Site), show.legend = F)+ #plot arrows linking trajectory points
  ...
  ylab("Temperature (ºC)")+ #add a y-axis label
  ggtitle("B - Trajectory PC1")+
...
Traj2_temp.plot <- ggplot()+ #create new ggplot object
  geom_path(data=daily_temps.df, aes(x=Date, y=Temp.avg, color=Site), linetype=2, show.legend = F)+ #plot path of daily average temperatures
  geom_point(data=trajectory_traj_plot.df, aes(x=Date, y=(Traj2/20) + 15), size=2, show.legend = F)+ #plot points showing trajectory point value on PC2 during each sampling date (rescaled to fit on same plot as temperature)
  geom_arrow_chain(data=trajectory_traj_plot.df, aes(x=Date, y=(Traj2/20) + 15, color=Site), show.legend = F)+ #plot arrows linking trajectory points
  theme_classic()+ #change the plot theme
  ...
  xlim(c(as.Date.character("2022-07-15"), as.Date.character("2022-08-20")))+ #standardize the x axis to focus on the sampling period
  ylab("Temperature (ºC)")+ #add a y-axis label
  ggtitle("C - Trajectory PC 2")+
  facet_wrap(~Site, nrow=1) #create a row of panels showing each stream individually

traj_temp.plots <- grid.arrange(full_trajectory.plot, Traj1_temp.plot, Traj2_temp.plot, layout_matrix=matrix(c(1,1,2,3), nrow=4, ncol=1))
ggsave("traj_temp.jpeg", traj_temp.plots, width=8, height=11, unit="in", dpi=300)
