...
register_stadiamaps("INSERT_YOUR_API_KEY") #add Stadia Maps api key
...
NHD_flowline.shp <- st_read(dsn="NHDPlusMA/NHDPlus02/NHDSnapshot/Hydrography/", layer="NHDFlowline") #read in shapefile with National Hydrology Database flowlines for the mid-Atlantic region
...
daily_temps.df <- read.delim("BrookTrout_Heatwaves_DailyTemps.txt") #read in the temperature logger data
daily_temps.df$Date <- as.Date(daily_temps.df$Date) #convert the date column in the temperature data frame to Date format
trout_sample_data.df <- read_excel("2022 trout sampling data.xlsx", sheet="HW trout field data") #read in the trout sampling data to get sampling dates
